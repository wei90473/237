    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="row pc_show">
                <div class="col-sm-12">
                    <h4 class="pull-left page-title">名額彙總表</h4>
                    <ol class="breadcrumb pull-right">
                        <li><a href="/admin">首頁</a></li>
                        <li class="active">報表列印</li>
                        <li class="active">名額彙總表</li>
                    </ol>
                </div>
            </div>
                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list pr-2"></i>名額彙總表</h3>
                        </div>

                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                        <form method="post" action="/admin/studyplan_quota_all/export" id="search_form">
                                        {{ csrf_field() }}
                                            <!-- 年度 -->
                                            ...
                                            <!-- 參訓機關 -->
                                            <div class="form-group row">
                                                <label class="col-sm-2 control-label text-md-right pt-2">參訓機關<span class="text-danger"></span></label>
                                                <div class="col-sm-10">
                                                    <div class="input-group bootstrap-touchspin number_box">
                                                        <select id="organ" name="organ" class="select2 form-control select2-single input-max">
                                                            <option value="0">此欄可空白</option>
                                                        </select>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div align="center">
                                                ...
                                                <label class="control-label text-md-right pt-2" style="font-size: 18px; color:red; ">若參訓機關未設定，則會列出所有機關的資料</label>
                                            </div>
                                        </form>
                                </div>
                            </div>
...
//跳出訊息視窗
    ...
    function getTimes(){
        $.ajax({
            ...
            dataType: "html",
            url: '/admin/studyplan_quota_all/gettime',
    ...
    function getOrgans()
    {
        $.ajax({
            ...
            headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            dataType: "html",
            url: '/admin/studyplan_quota_all/getorgan',
            data: {
                yerly: $('#yerly').val(),
                times: $('#times').val()
                },
            success: function(response){
                ...
                let tempHTML = "";
                tempHTML +="<option value='0'>請選擇參訓機關</option>";
                for(let i=0; i<dataArr.length; i++) {
                    tempHTML += "<option value='"+dataArr[i].organ+"'>"+dataArr[i].organ+'-'+dataArr[i].機關+"</option>";
                }
                $("#organ").html(tempHTML);
            },
            error: function(){
