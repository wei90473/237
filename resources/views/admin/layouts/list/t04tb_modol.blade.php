...
<div id="t04tb" class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
                    ...
                    <div class="search-float">
                        <div class="float-md mobile-100 row mr-1 mb-3">      
                            <div class="input-group col-4" style="padding-bottom:0px;">
                                ...
                                <input type="text" name="modal_class" class="form-control">
                            </div>  
                            <div class="input-group col-6" style="padding-bottom:0px;">
                                <div class="input-group-prepend">
                                    <label class="input-group-text">班別名稱</label>
                                </div>
                                <input type="text" name="modal_class_name" class="form-control">
                            </div> 
                            <button type="button" onclick="queryT04tb()" class="btn mobile-100 mb-3 mb-md-0"><i class="fa fa-search fa-lg pr-1"></i>搜尋</button>
                            <button type="button" onclick="resetT04tbCondition()" class="btn mobile-100 mb-3 mb-md-0">重設條件</button>                                                                                                                                                                                                                   
                        </div>
...
function queryT04tb()
{
    ...
    var class_no = $("input[name=modal_class]").val();
    var class_name = $("input[name=modal_class_name]").val();
    ...
    $.ajax({
        ...
        if (response.total > 0){
            paginateT04tb(response.queryData, response.total);
        }else{
...
function paginateT04tb(queryData, total) {
    ...
    container.pagination({
        ...
        callback: function(data, pagination) {
            ...
            for (var i in data) {
                class_html += "<tr>" + 
                                "<td class='text-center'>" + "<button data-dismiss='modal' type='button' onclick='chooseT04tb(\"" + data[i].t01tb.class + '","' + data[i].term  + '","' + data[i].t01tb.name +  "\")' class='btn btn-primary'>選擇</button>" + "</td>" + 
