                ...
                <div class="col-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title"><i class="fa fa-list pr-2"></i>班別資料查詢</h3>
                        </div>

                        <div class="card-body">
                            <div class="row">
                                <div class="col-12">
                                    <div class="float-md row mb-2">
                                    <!--** 維護追蹤培訓班別 -->
                                    <!--button type="button" id="trainClass" name="trainClass" onclick="javascript:maintainTrainClass();" class="btn btn-primary btn-sm mb-1" style=" margin-right:5px;">維護追蹤培訓班別</button-->
                                    <!-- 批次增刪班別 -->
                                    <a href="/admin/classes/batch">
                                        <button type="button" id="BatchDel" name="BatchDel" class="btn btn-primary btn-sm mb-1" style=" margin-right:5px;">批次增刪班別</button>
                                     </a>
                                    <!-- <button type="button" id="BatchDel" name="BatchDel" data-target="#BatchModal" class="btn btn-primary btn-sm mb-1" style=" margin-right:5px;">批次增刪班別</button> -->
                                    <!-- 維護跨區報名機關 -->
                                    <button type="button" id="crossArea" name="crossArea" onclick="javascript:maintainCrossArea();" class="btn btn-primary btn-sm mb-1" style=" margin-right:5px;">維護跨區報名機關</button>

                                    <!-- 維護課程簡介 -->
                                    <!-- <button type="button" id="classIntro" name="classIntro" onclick="javascript:maintainClassIntro();" class="btn btn-primary btn-sm mb-1" style=" margin-right:5px;">維護課程簡介</button> -->
                
                                    <!-- 匯出入班號 -->
                                    <!-- <button type="button" id="ClassIO" name="ClassIO" data-target="#ClassIOModal" class="btn btn-primary btn-sm mb-1" style=" margin-right:5px;">匯出入班號</button> -->
                                        ...
                                        <form method="get" id="search_form">
                                            ...
                                            <div class="float-md mobile-100 row mr-1 mb-3 ">
                                                ...
                                                <div class="input-group col-6">
                                                    ...
                                                    <input type="text" id="class" name="class" class="form-control" autocomplete="off" value="{{ isset($queryData['class'])?$queryData['class']:'' }}">
                                                </div>
                                                    <!-- 辦班院區 -->
                                                <div class="input-group col-6">
                                                    <div class="input-group-prepend">
                                                    ...
                                                    <select class="form-control select2 " name="branch">
                                                        ...
                                                        @foreach(config('app.branch') as $key => $va)
                                                            <option value="{{ $key }}" {{ (isset($queryData['branch'])?$queryData['branch']:'' )== $key? 'selected' : '' }}>{{ $va }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                                ...
                                                <div class="input-group col-6">
                                                    ...
                                                    <select class="form-control select2 " name="process" onchange="showclient()">
                                                        ...
                                                        @foreach(config('app.process') as $key => $va)
                                                            <option onclick="ChangeDisabled({{ $key }})" value="{{ $key }}" {{ (isset($queryData['process'])?$queryData['process']:'' ) == $key? 'selected' : '' }}>{{ $key.$va }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <!-- 委訓機關 -->
                                                <div class="input-group col-6" id="clientclass" style="display: none;">
                                                    <div class="input-group-prepend">
                                                        <span class="input-group-text">委訓單位</span>
                                                    </div>
                                                    <input type="text" id="commission" name="commission" class="form-control" autocomplete="off" value="{{ isset($queryData['commission'])?$queryData['commission']:'' }}">
                                                </div>
                                            </div>
                                            <!-- 訓練性質 -->
                                            <div class="float-md mobile-100 row mr-1 mb-3">
                                                <div class="input-group col-4">
                                                    ...
                                                    <select class="form-control select2 " name="traintype">
                                                        ...
                                                        @foreach(config('app.traintype') as $key => $va)
                                                            <option value="{{ $key }}" {{ (isset($queryData['traintype'])?$queryData['traintype']:''  )== $key? 'selected' : '' }}>{{ $key.$va }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <!-- 班別性質 -->
...
<!-- 進階/簡易搜尋結束 -->
                                        ...
                                        <div class="float-left">
                                            ...
                                            <!-- Sort -->
                                            <button type="button" class="btn mobile-100 mb-3 mb-md-0" data-toggle="modal" data-target="#SortModal">排序</button>
                                            <!-- 新增班別資料 -->
                                            <a href="/admin/classes/create">
                                                <button type="button" class="btn btn-primary btn-sm mb-3 mb-md-0"><i class="fa fa-plus fa-lg pr-2"></i>新增班別資料</button>
                                            </a>
                                        </div>    
                                        </form>
                                    </div>
                                            ...
                                            @foreach($data as $va)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        <a href="/admin/classes/{{ $va->class }}/edit" data-placement="top" data-toggle="tooltip" data-original-title="編輯">
                                                            <i class="fa fa-pencil">編輯</i>
                                                        </a>
                                                    </td>
                                                    <td>{{ $va->class }}</td>
                                                    <td>{{ $va->name }}</td>
                                                    <td>{{ $typeList[$va->type]['name'] }}</td>
                            ...
                            @foreach($ranklist as $va)
                                ...
                                <tr draggable="true">												
									...
									<td>
                                     ...
                                     <input type="hidden" name="{{$va->class}}" value="{{ $va->rank==''?$rank:$va->rank }}">
                                </tr>
                            <?php $rank= $rank+1; ?>      
                            @endforeach
                            @endif
