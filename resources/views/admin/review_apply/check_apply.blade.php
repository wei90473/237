            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h3 class="card-title"><i class="fa fa-list pr-2"></i>參訓檢核</h3>
                    </div>
                    ...
                    <div class="card-body">
                            ...
                            <form>     
                                <div class="search-float">
                                    ...
                                    <div class="float-md mobile-100 row mr-1 mb-3">      
                                        <div class="input-group col-5">
                                            <div class="input-group-prepend">
                                            <label class="input-group-text">檢核班別</label>
                                            </div>
                                            <select id="check_class" name="check_class" >
                                                @if (isset($check_t01tb))
                                                    <option value="{{$check_t01tb->class}}">{{$check_t01tb->name}}</option>
                                                @endif
                                            </select>
                                        </div>                                          
                                    </div>
                        ...
                        <div class="table-responsive">                  
                            <table id="data_table" class="table table-bordered mb-0">
                                ...
                                <tbody>
                                    @if (isset($check_t27tbs))
                                        @foreach($check_t27tbs as $t27tb)
                                        <tr>
                                            <td>{{ $t27tb->m02tb->cname }}</td>
                                            <td>{{ $t27tb->class }}</td>
                                            <td>{{ $t27tb->term }}</td>
                                            <td>{{ $t27tb->t01tb_name }}</td>
                                            <td>{{ $t27tb->t04tb_sdate}}</td>
                                        </tr>
                                        @endforeach
                                    @endif
                                </tbody>
