...
class ItineracyThemeController extends Controller
{
    private $type = '1'; //主題
    /**
    ...
    public function __construct(ItineracyService $itineracyService, User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
            ...
            if(in_array('itineracy_theme', $user_group_auth)){
                return $next($request);
            }else{
    ...
    public function index(Request $request){
        ...
        if(empty($request->all()) ){
           return view('admin/itineracy_theme/list', compact('queryData'));
        }
        $data = $this->itineracyService->getItineracyList($queryData);
        return view('admin/itineracy_theme/list', compact('data','queryData'));
    }
    ...
    public function destroy($code) {
        ...
        if($del){
            return redirect('admin/itineracy_theme')->with('result', '1')->with('message', '刪除成功!');
        } else {
