...
class ClassroomUsageList extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('admin/classroom_usage_list/list', compact(['classrooms']));
    }


    public function export(Request $request)
    {
        if ($request->queryType == 'class'){

        }else if ($request->queryType == 'date'){
            $this->validate($request, ['trainingSdate' => 'required', 'trainingEdate' => 'required']);
        }

        $queryData = $request->only(['class', 'class_name', 'term', 'trainingSdate', 'trainingEdate', 'branch', 'site', 'teacher_id', 'course_name', 'commission']);
        ...
        $sheet->getCell('C4')->setValue('班別名稱');
        $sheet->getCell('D4')->setValue('課程名稱');
        $sheet->getCell('E4')->setValue('講座');
        $sheet->getCell('F4')->setValue('起訖時間');
        $sheet->getCell('G4')->setValue('日期');
        $sheet->getCell('H4')->setValue('班務人員');
        ...
        foreach ($sites as $Csrms){
            foreach ($Csrms as $site => $t06tbGroup){
                $merge = false;
                $t06tbGroupCount = $t06tbGroup->groupBy('date')->map(function($dateGroup){
                    return $dateGroup->groupBy(function($t06tb){
                        return $t06tb->class.'_'.$t06tb->term.'_'.$t06tb->course;
                    })->count();
                })->sum();
                ...
                foreach ($t06tbGroup as $key => $t06tb){
                    ...
                    $sheet->getCell('B'.$row)->setValue($t06tbGroupCount);
        ...
        $sheetHelper = new \App\Helpers\SheetHelper();

        $fileName = '教室使用一覽表';
        if ($request->doctype == 'xlsx'){
            $xlsxFilePath = $sheetHelper->exportXlsx($spreadSheet, $fileName);
        }elseif ($request->doctype == 'ods'){
            $xlsxFilePath = $sheetHelper->exportXlsx($spreadSheet, $fileName, true);
            $sheetHelper->convertToOds($xlsxFilePath, $fileName);
        }
    }
    ...
    public function getRoomData($queryData)
    {
        ...
        if (!empty($queryData['trainingSdate'])){
            $nantouCsrms->where('t06tb.date', '>', $queryData['trainingSdate']);
            $nantouCsrmsDefault->where('t06tb.date', '>', $queryData['trainingSdate']);
            $taipeiCsrms->where('t06tb.date', '>', $queryData['trainingSdate']);
            $taipeiCsrmsDefault->where('t06tb.date', '>', $queryData['trainingSdate']);
        }


        if (!empty($queryData['trainingSdate'])){
            $nantouCsrms->where('t06tb.date', '<', $queryData['trainingEdate']);
            $nantouCsrmsDefault->where('t06tb.date', '<', $queryData['trainingEdate']);
            $taipeiCsrms->where('t06tb.date', '<', $queryData['trainingEdate']);
            $taipeiCsrmsDefault->where('t06tb.date', '<', $queryData['trainingEdate']);
        }
        ...
        if (!empty($queryData['branch'])){
            if ($queryData['branch'] == 1){
                ...
                $nantouCsrms = $nantouCsrms->union($nantouCsrmsDefault)->orderBy('date')->orderBy('stime')->get()->groupBy('site');
            }

            if ($queryData['branch'] == 2){
                $taipeiCsrms = collect([]);
            }else{
                $taipeiCsrms = $taipeiCsrms->union($taipeiCsrmsDefault)->orderBy('date')->orderBy('stime')->get()->groupBy('site');
            }
        }
