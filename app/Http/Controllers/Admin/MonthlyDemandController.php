...
class MonthlyDemandController extends Controller
{
    public function __construct(User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
            ...
            if(in_array('monthly_demand', $user_group_auth)){
                return $next($request);
            }else{
    ...
    {
        return view('admin/monthly_demand/list');
    }
    ...
    */
    ...
    public function export(Request $request)
    {
        ...
        //西元年
        $YYYY = substr($yearmonth,0,4);
        //民國年
        $YYY = (substr($yearmonth,0,4)-'1911');
        //月份
        $MM = ltrim(substr($yearmonth,4,2),'0');
        //西元年月日,日期為01日起
        $YYYYMMDD = $yearmonth.'01';
        //民國年月
        $YYYMM = (substr($yearmonth,0,4)-'1911').substr($yearmonth,4,2);
        ...
        /*取得起迄日期/週,週數*/
        $sqlWeek="SELECT str_to_date('".$YYYYMMDD."', '%Y%m%d') AS sYYYYMMDD,
                        LAST_DAY(str_to_date('".$YYYYMMDD."', '%Y%m%d')) AS eYYYYMMDD,
                        WEEK(str_to_date('".$YYYYMMDD."', '%Y%m%d'),0)  AS sweek,
                        WEEK(LAST_DAY(str_to_date('".$YYYYMMDD."', '%Y%m%d')),0) AS eweek,
                        WEEK(LAST_DAY(str_to_date('".$YYYYMMDD."', '%Y%m%d')),0) - WEEK(str_to_date('".$YYYYMMDD."', '%Y%m%d'),0) + 1	AS weeknum,
                        YEARWEEK(str_to_date('".$YYYYMMDD."', '%Y%m%d')) AS YEARWEEK
                    FROM DUAL ";
        $reportlistWeek = DB::select($sqlWeek);
        $dataArrWeek = json_decode(json_encode($reportlistWeek), true);

        //Clone 建立sheet
        $count=1;
        for ($k=1; $k < $dataArrWeek[0]['weeknum']; $k++) {
            $sheet = clone $objPHPExcel->getSheet(0);
            $sheet->setTitle('Sheet'.($k+1));
        ...
        $startWeekDate ='';
        $endWeekDate='';
        for ($w=0; $w < $dataArrWeek[0]['weeknum']; $w++) {
            //指定sheet
            $objActSheet = $objPHPExcel->getSheet($w);
            $objActSheet->setTitle(''.$YYY.'年'.$MM.'月第'.($w+1).'週');

            //查詢 2019年17的星期日/六的日期, %w 周的阿拉伯顯示 ：0（星期日）-6（星期六）
            //2019170
            //2019176

            //取得週別的日期範圍
            $sqlDate="  SELECT  DATE_FORMAT(str_to_date(
                                CONCAT(YEARWEEK(DATE_ADD(str_to_date('".$YYYYMMDD."', '%Y%m%d'), INTERVAL '".$w."' WEEK),0),' 0')
                                , '%X%V %w'),'%Y%m%d') AS sdate,
                                DATE_FORMAT(str_to_date(
                                CONCAT(YEARWEEK(DATE_ADD(str_to_date('".$YYYYMMDD."', '%Y%m%d'), INTERVAL '".$w."' WEEK),0),' 6')
                                , '%X%V %w'),'%Y%m%d') AS edate
                        FROM DUAL";
            $reportlistDate = DB::select($sqlDate);
            $dataArrDate = json_decode(json_encode($reportlistDate), true);

            //轉換民國年月日
            $startDate = (substr($dataArrDate[0]['sdate'],0,4)-'1911').substr($dataArrDate[0]['sdate'],4,4);
            $endDate = (substr($dataArrDate[0]['edate'],0,4)-'1911').substr($dataArrDate[0]['edate'],4,4);

            //標題
            $sql="SELECT CONCAT('行政院人事行政總處公務人力發展學院','".$YYY."','年','".$MM."','月',
                        ' 第',
                        '".($w+1)."'
                ...
                FROM (SELECT DATE_ADD(A.DATE1,INTERVAL 0 DAY) AS DATE1,
                        ...
                        FROM (
                                SELECT '".($dataArrDate[0]['sdate'])."' AS DATE1
                                FROM DUAL ) A
                            ) B ";
                ...
                FROM ( SELECT '教室' AS type, '上午' AS time FROM DUAL
                        ...
                        (
                            ...
                            FROM (  SELECT t37.date, m14.type,
                                    ...
                                    WHERE t37.date between '".$startDate."' AND '".$endDate."'
                                    AND substr(t37.date,1,5) = '".$YYYMM."'
                                    AND t37.type = '2'
                                ...
                                SELECT CASE WHEN T.sincnt > 0 OR T.donecnt > 0 OR T.dtwocnt > 0 THEN '住宿及休閒設施'
                                    ...
                                    T.date, CCC.SORT
                                        FROM (SELECT t23.class, t23.term, date, sincnt, donecnt, dtwocnt,
                                                    ...
                                                    WHERE t23.date between '".$startDate."' AND '".$endDate."'
                                                    AND substr(t23.date,1,5) = '".$YYYMM."'
                                                ) T LEFT JOIN (  SELECT (@rownum := @rownum + 1) AS SORT, CC.class, CC.term, CC.site
                                                                            FROM (
                                                                                        SELECT C.name, C.site, C.class, C.term
                                                                                            FROM (
        ...
        $RptBasic->exportfile($objPHPExcel,"2",$request->input('doctype'),"每月需求表");
        //$obj: entity of file
