...
class TrainingEvaluation9395Controller extends Controller
{
    public function __construct(User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
          ...
          if(in_array('training_evaluation_93_95', $user_group_auth)){
            return $next($request);
          }else{
    ...
    {
        //return view('admin/training_evaluation_93_95/list');
        $classArr = $this->getclass();
        $result = '';
        return view('admin/training_evaluation_93_95/list', compact('classArr', 'result'));
    }
    ...
    訓練成效評估表 CSDIR5011
    參考Tables:
    使用範本:L15A.docx, L15B.docx (基本資料列印選項,  L15A 不要包含, L15B 要包含 )
    */
    /**
     * 列印檔案
     *
    ...
    public function export(Request $request)
    {
        ...
        */
        if($ratioInfo!="1")
          $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('../example/'.iconv('UTF-8', 'BIG5', 'L15A').'.docx');
        else
          $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('../example/'.iconv('UTF-8', 'BIG5', 'L15B').'.docx');
        ...
        */
        if($ratioInfo!="1")
        $outputname="訓練成效評估表(93~95)-不包含基本資料";
        else
        $outputname="訓練成效評估表(93~95)-要包含基本資料";
