...
class ItineracyController extends Controller
{
    private $type = '2'; //類別
    ...
    public function __construct(ItineracyService $itineracyService, User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
            ...
            if(in_array('itineracy', $user_group_auth)){
                return $next($request);
            }else{
    ...
    public function index(Request $request){
        ...
        if(empty($request->all()) ){
           return view('admin/itineracy/list', compact('queryData'));
        }
        $data = $this->itineracyService->getItineracyList($queryData);
        return view('admin/itineracy/list', compact('data','queryData'));
    }
    ...
    public function destroy($code) {
        ...
        if($del){
            return redirect('admin/itineracy')->with('result', '1')->with('message', '刪除成功!');
        } else {
