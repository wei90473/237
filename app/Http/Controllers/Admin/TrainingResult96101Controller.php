...
class TrainingResult96101Controller extends Controller
{
    public function __construct(User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
            ...
            if(in_array('training_result_96_101', $user_group_auth)){
                return $next($request);
            }else{
    ...
    {
        //return view('admin/training_result_96_101/list');
        $classArr = $this->getclass();
        $result = '';
        return view('admin/training_result_96_101/list', compact('classArr', 'result'));
    }
    ...
    public function getclass() {
            $sql = "SELECT DISTINCT t53tb.class, t01tb.name
                     ...
                     WHERE t53tb.times<>''
                     AND SUBSTRING(t53tb.class,1,3) between '096' AND '101'
    ...
    */
    ...
    public function export(Request $request)
    {
       ...
       $sql="SELECT
                ...
                FROM
                (
                    SELECT  'q11' AS key1, 'Q11.我認為本次研習所訂研習目標符合組織或個人之需求' AS caption
                    UNION ALL
                    SELECT  'q12' AS key1, 'Q12.我認為本次研習的課程規劃有助於達成研習目標' AS caption
                    UNION ALL
                    SELECT  'q13' AS key1, 'Q13.我認為本次研習的課程內容與個人辦理之業務具有相關性' AS caption
                    UNION ALL
                    SELECT  'q14' AS key1, 'Q14.我認為參加本次研習對增進個人工作所需知能有助益' AS caption
                    UNION ALL
                    SELECT  'q15' AS key1, 'Q15.我認為本研習班可推薦給其他符合參訓資格之同仁參加' AS caption
                    UNION ALL
                    SELECT  'q21' AS key1, 'Q21.我對於本班輔導員的學習輔導與相關服務感到滿意' AS caption
                    UNION ALL
                    SELECT  'q22' AS key1, 'Q22.我對於本次研習的教材及相關資料感到滿意' AS caption
                    UNION ALL
                    SELECT  'q23' AS key1, 'Q23.我對於本次研習的教學環境與設備感到滿意' AS caption
                ) A
                INNER JOIN t75tb t75tb ON 1 = 1
            WHERE t75tb.class= '".$class."'
            AND t75tb.term= '".$term."'
            AND t75tb.times= IFNULL('".$times."',t75tb.times)
    ...
    $sqlTitle="SELECT DISTINCT t53tb.class, t01tb.name, t53tb.term, t53tb.times,
                    ...
                    , '期訓練成效評估結果統計表' ,
                                                            ...
                                                            ,')')  AS TITLE
                FROM t53tb INNER JOIN t01tb ON t53tb.class = t01tb.class
                WHERE t53tb.times<>'' AND SUBSTRING(t53tb.class,1,3) between '096' AND '101'
    ...
    $sqlRemark2="SELECT CONCAT('2.本次研習學員滿意度總平均為', totper, '分') AS REMARK2A,
                        CONCAT('，其中「研習規劃」(佔25％)滿意度平均為', conper,
                               '、「學習輔導」(佔25％)滿意度平均為', worper) AS REMARK2B,
                        CONCAT('、「講座授課」(佔50％)滿意度平均為', teaper,
    ...
    if(empty($dataArrRemark1)){
        ...
        $objActSheet->getCell('A15')->setValue($objRichText);
    }
    ...
    if(empty($dataArrRemark2)){
        $objActSheet->setCellValue('A16', '2.本次研習學員滿意度總平均為0.00分，其中「研習規劃」(佔25％)滿意度平均為0.00、「學習輔導」(佔25％)滿意度平均為0.00');
        $objActSheet->setCellValue('A17', '、「講座授課」(佔50％)滿意度平均為0.00，「講座授課」統計結果詳附表。');
    }else{
        ...
        $objActSheet->setCellValue('A17', $dataArrRemark2[0]['REMARK2C']);
    ...
    $objActSheet->setCellValue('A18', $REMARK3);
    ...
    $dsl = array(new \PHPExcel_Chart_DataSeriesValues('String', '固定題目統計!$C$3', NULL, 1));
    $xal = array(new \PHPExcel_Chart_DataSeriesValues('String', '固定題目統計!$C$4:$C$11', NULL, 100));
    $dsv = array(new \PHPExcel_Chart_DataSeriesValues('Number', '固定題目統計!$O$4:$O$11', NULL, 100));
    //顯示數值
    ...
    */
    ...
    $RptBasic = new \App\Rptlib\RptBasic();
    $RptBasic->exportfile($objPHPExcel,"3",$request->input('doctype'),"訓練成效評估結果統計圖表(96)");
    //$obj: entity of file
