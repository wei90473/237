...
use App\Services\Teacher_receptionService;
...
class Teacher_receptionController extends Controller
{
    /**
     * Teacher_receptionController constructor.
     * @param Teacher_receptionService $teacher_receptionService
     */
    public function __construct(Teacher_receptionService $teacher_receptionService, User_groupService $user_groupService)
    {
        $this->teacher_receptionService = $teacher_receptionService;
        $this->user_groupService = $user_groupService;
        //檢查權限
        $this->middleware(function($request, $next){
            $user_data = \Auth::user();
            $user_group_auth = $this->user_groupService->getUser_auth($user_data->user_group_id);
            if(in_array('teacher_reception', $user_group_auth)){
                return $next($request);
            }else{
                return redirect('admin/home')->with('result', '0')->with('message', $this->user_group_msg);
            }
        });
        setProgid('teacher_reception');
    ...
    {
        $sponsor = $this->teacher_receptionService->getSponsor();
        //年
        ...
        if($queryData['search'] != 'search' ){
          ...
          $data = $this->teacher_receptionService->getTeacher_receptionList($queryData2);
        }else{
          $data = $this->teacher_receptionService->getTeacher_receptionList($queryData);
        }

        // $begin_date="2019-4-22";
        // $end_date="2019-5-15";
        ...
        return view('admin/teacher_reception/list', compact('data', 'queryData', 'sponsor'));
    }
    ...
    public function detail(Request $request)
    {
        ...
        $class_data = $this->teacher_receptionService->getClass($queryData);
        ...
        $teacher_list = $this->teacher_receptionService->getDetailList($queryData);
        // dd($teacher_list);

        return view('admin/teacher_reception/detail', compact('data', 'class_data', 'teacher_list'));
    }
    ...
    public function car_detail(Request $request)
    {
        ...
        if($queryData['search'] != 'search')
        {
            $queryData2['date'] = 'none';
          	$data = $this->teacher_receptionService->getCarList($queryData2);
        }else{
            if(empty($queryData['date'])){
                return back()->with('result', '0')->with('message', '請輸入日期');
            }else{
            	$data = $this->teacher_receptionService->getCarList($queryData);

              $car_data = $this->teacher_receptionService->getLicenseData();
              $queryData['auto'] = $request->get('auto');

              if($queryData['auto'] == 'auto'){

                foreach($data as $data_row){
                  if(array_key_exists($data_row['license_plate'], $car_data)){
                    $car_data[$data_row['license_plate']]['count'] = $car_data[$data_row['license_plate']]['count']+1;
                  }

                }

                foreach($data as & $data_row2){
                  if(empty($data_row2['license_plate'])){
                    foreach($car_data as $car_data_row){
                      if($car_data_row['count'] < '3' && empty($data_row2['license_plate'])){
                        $data_row2['license_plate'] = $car_data_row['license_plate'];
                        $car_data[$car_data_row['license_plate']]['count'] = $car_data[$car_data_row['license_plate']]['count']+1;
                      }
                    }
                  }
                ...
                $car_1 = config('app.car_1');
                $car_A = config('app.car_A');
                unset($car_1['Z']);
                unset($car_A['Z']);
                $carType1 = array();
                $carType2 = array();
                foreach($car_1 as $car_1_key => $car_1_row){
                	$carType1[$car_1_key] = array(
                		'car' => $car_1_key,
                		'count' => '0',
                	);
                }
                foreach($car_A as $car_A_key => $car_A_row){
                	$carType2[$car_A_key] = array(
                		'car' => $car_A_key,
                		'count' => '0',
                	);
                }
                ...
                foreach($data as $data_row){
                	if($data_row['type'] == '1'){
                		if(array_key_exists($data_row['car'], $car_1)){
	                    $carType1[$data_row['car']]['count'] = $carType1[$data_row['car']]['count']+1;
	                  }
                	}
                	if($data_row['type'] == '2'){
                		if(array_key_exists($data_row['car'], $car_A)){
	                    $carType2[$data_row['car']]['count'] = $carType2[$data_row['car']]['count']+1;
	                  }
                	}
                ...
                foreach($data as & $data_row2){
                  if(empty($data_row2['car'])){
                  	if($data_row2['type'] == '1'){
                  		foreach($carType1 as $carType1_row){
	                      if($carType1_row['count'] < '3' && empty($data_row2['car'])){
	                        $data_row2['car'] = $carType1_row['car'];
	                        $carType1[$carType1_row['car']]['count'] = $carType1[$carType1_row['car']]['count']+1;
	                      }
	                    }
                  	}
                  	if($data_row2['type'] == '2'){
                  		foreach($carType2 as $carType2_row){
	                      if($carType2_row['count'] < '3' && empty($data_row2['car'])){
	                        $data_row2['car'] = $carType2_row['car'];
	                        $carType2[$carType2_row['car']]['count'] = $carType2[$carType2_row['car']]['count']+1;
	                      }
	                    }
                  	}
                  }
        ...
        return view('admin/teacher_reception/car_detail', compact('data', 'queryData', 'car_data'));
    }
    ...
    public function car_update(Request $request)
    {
        ...
        $car_count = array();
        $car = 'N';
        foreach($data['update_id'] as $id_row){
          $fields = array();
          if(isset($data[$id_row.'_price'])){
            $fields['price'] = $data[$id_row.'_price'];
          }
          if(isset($data[$id_row.'_remark'])){
            $fields['remark'] = $data[$id_row.'_remark'];
          }
          if(isset($data[$id_row.'_license_plate'])){
            $fields['license_plate'] = $data[$id_row.'_license_plate'];
          }
          if(isset($data[$id_row.'_car']) && $data[$id_row.'_car'] != ''){
            $fields['car'] = $data[$id_row.'_car'];
            if(!array_key_exists($data[$id_row.'_car'],$car_count)){
              $car_count[$data[$id_row.'_car']]['count'] = "1";
            }else{
              $car_count[$data[$id_row.'_car']]['count'] = $car_count[$data[$id_row.'_car']]['count']+"1";
            }
            if($car_count[$data[$id_row.'_car']]['count'] > "3"){
              $fields['car'] = null;
              $car = 'Y';
            }
          }else{
          ...
          if(checkNeedModifyLog('teacher_reception')){
              $olddata = Teacher_car::where('id', $id_row)->get()->toarray();
          }

          Teacher_car::where('id', $id_row)->update($fields);

          $sql = DB::getQueryLog();
          if(checkNeedModifyLog('teacher_reception')){
              $nowdata = Teacher_car::where('id', $id_row)->get()->toarray();
              createModifyLog('U','Teacher_car',$olddata,$nowdata,end($sql));
          }

        }
        // dd($car_count);
        if($car == 'Y'){
          return redirect("/admin/teacher_reception/car_detail?search=search&date={$data['date']}")->with('result', '0')->with('html_message', "一車三人為上限，超過不可設定!");
        }else{
          return redirect("/admin/teacher_reception/car_detail?search=search&date={$data['date']}")->with('result', '1')->with('message', '儲存成功!');
        }
    ...
    public function room_detail(Request $request)
    {
        ...
        {
            $queryData2['date'] = 'none';
          	$data = $this->teacher_receptionService->getRoomList($queryData2);
        }else{
            if(empty($queryData['date'])){
                return back()->with('result', '0')->with('message', '請輸入日期');
            }else{
            	$data = $this->teacher_receptionService->getRoomList($queryData);
                // echo '<pre style="text-align:left;">' . "\n";
                // print_r($data);
                // echo "\n</pre>\n";
                // die();
        ...
        return view('admin/teacher_reception/room_detail', compact('data', 'queryData'));
    }
    ...
    public function food_detail(Request $request)
    {
        ...
        {
            $queryData2['date'] = 'none';
          	$data = $this->teacher_receptionService->getFoodList($queryData2);
        }else{
            if(empty($queryData['date'])){
                return back()->with('result', '0')->with('message', '請輸入日期');
            }else{
            	$data = $this->teacher_receptionService->getFoodList($queryData);
                // echo '<pre style="text-align:left;">' . "\n";
                // print_r($data);
                // echo "\n</pre>\n";
                // die();
        ...
        return view('admin/teacher_reception/food_detail', compact('data', 'queryData'));
    }
    ...
    public function changehire(Request $request, $id)
    {
      ...
      $ClassWeek_data = $this->teacher_receptionService->getClassWeek($queryData);

      $TeacherByWeek_data = $this->teacher_receptionService->getTeacherByWeek($queryData);
      ...
      if (!empty($TeacherByWeek_data)) {
          ...
          return redirect("/admin/teacher_reception/detail?class={$ClassWeek_data['class']}&term={$ClassWeek_data['term']}&sdate={$ClassWeek_data['sdate']}")->with('result', '1')->with('message', '修改成功!');
      } else {
    ...
    public function edit1($id)
    {
        ...
        if(isset($data_array[2])){
          $goBack['type'] = $data_array[2];
          $goBack['date'] = $data_array[3];
        }
        ...
        $idno_data = $this->teacher_receptionService->getIdno($queryData);
        if(!empty($idno_data)){
        ...
        $teachDate = $this->teacher_receptionService->getTeachDate($queryData);
        $RoomDate = $this->teacher_receptionService->getRoomDate($queryData);
        // dd($RoomDate);
        $TeacherByWeek_data['before_day'] = $teachDate['before_day'];
        $TeacherByWeek_data['after_day'] = $teachDate['after_day'];
        $RoomNumber = $this->teacher_receptionService->getRoom();
        ...
        return view('admin/teacher_reception/form1', compact('data', 'ClassWeek_data', 'idno_data', 'TeacherByWeek_data', 'RoomDate', 'RoomNumber', 'goBack'));
    }
    ...
    public function edit2($id)
    {
        ...
        $TeacherByWeek_data = $this->teacher_receptionService->getTeacherByWeek($queryData);
        $CarDate = $this->teacher_receptionService->getCarDate($queryData);
        ...
        return view('admin/teacher_reception/form2', compact('data', 'ClassWeek_data', 'idno_data', 'TeacherByWeek_data', 'goBack'));
    }
    ...
    public function edit3($id)
    {
        ...
        $TeacherByWeek_data = $this->teacher_receptionService->getTeacherByWeek($queryData);
        $FoodDate = $this->teacher_receptionService->getFoodDate($queryData);
        ...
        //$foodIsset = $query->where('class_weeks_id', $queryData['class_weeks_id'])->where('idno', $queryData['idno'])->where('date', '>=',$queryData['sdate'])->where('date', '<=',$queryData['edate'])->orderBy('date', 'asc')->orderBy('date', 'asc')->get()->toArray();
        $foodIsset = $query->where('class_weeks_id', $queryData['class_weeks_id'])->where('idno', $queryData['idno'])->where('date', '>=',$queryData['sdate'])->where('date', '<=',$queryData['edate'])->orderBy('date', 'asc')->orderBy('date', 'asc')->get()->toArray();
        if(empty($foodIsset)){
          $ClassWeek_data['data_isset'] = 'N';
        }
        ...
        return view('admin/teacher_reception/form3', compact('data', 'ClassWeek_data', 'idno_data', 'TeacherByWeek_data', 'goBack'));
    }
    ...
    public function edit4($id)
    {
        ...
        return view('admin/teacher_reception/form4', compact('data', 'ClassWeek_data', 'idno_data', 'TeacherByWeek_data', 'goBack'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        if(empty($TeacherByWeek_data)){
          ...
          if($data['type'] == 'teacher_room'){
              ...
              if(isset($data['room_no'])){
                $teacherByWeek_fields['room_no'] = $data['room_no'];
                $data_update = 'Y';
              }
        ...
        if($data['type'] == 'teacher_room'){
          ...
          $this->teacher_receptionService->change_room($queryData);
          $teachDate = $this->teacher_receptionService->getTeachDate($queryData);
          // dd($data);
          ...
          if(isset($data['confirm_date'])){
            foreach($data['confirm_date'] as $row){
              ...
              if(isset($data[$row])){
                foreach($data[$row] as $room_period){
