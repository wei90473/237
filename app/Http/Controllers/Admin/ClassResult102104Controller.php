...
class ClassResult102104Controller extends Controller
{
    public function __construct(User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
            ...
            if(in_array('class_result_102_104', $user_group_auth)){
                return $next($request);
            }else{
    ...
    {
        return view('admin/class_result_102_104/list');
    }

    /*
    年度各班期訓練成效評估統計表(102) CSDIR5033
    參考Tables:
    使用範本:L11A.xlsx, L11B.xlsx (區別是有無行政服務欄位項目)
    */
    /**
     * 列印檔案
     *
    ...
    public function export(Request $request)
    {
        ...
        if($checkboxservice!="2"){
            $sql="SELECT
                ...
                t57tb.worper as worper,
                    t57tb.offper AS offper,
                t57tb.teaper as teaper,
                t57tb.totper as totper,
                (
                    CASE
                        WHEN (t57tb.conper + t57tb.worper + t57tb.offper + t57tb.teaper + t57tb.totper)=0 THEN 0
                        ELSE (t57tb.conper + t57tb.worper + t57tb.offper + t57tb.teaper + t57tb.totper) /
                            (
                            (CASE WHEN t57tb.conper=0 THEN 0 ELSE 1 END)+
                            (CASE WHEN t57tb.worper=0 THEN 0 ELSE 1 END)+
                            (CASE WHEN t57tb.offper=0 THEN 0 ELSE 1 END)+
                            (CASE WHEN t57tb.teaper=0 THEN 0 ELSE 1 END)+
                            (CASE WHEN t57tb.totper=0 THEN 0 ELSE 1 END)
                            )
                    END ) AS total_AVG
            From t57tb
        ...
        $RptBasic->exportfile($objPHPExcel,"2",$request->input('doctype'),"年度各班期訓練成效評估統計表(102)");
        //$obj: entity of file
