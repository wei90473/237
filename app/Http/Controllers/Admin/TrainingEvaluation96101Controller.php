...
class TrainingEvaluation96101Controller extends Controller
{
    public function __construct(User_groupService $user_groupService)
    {
        ...
        $this->middleware(function($request, $next){
            ...
            if(in_array('training_evaluation_96_101', $user_group_auth)){
                return $next($request);
            }else{
    ...
    {
        //return view('admin/training_evaluation_96_101/list');
        $classArr = $this->getclass();
        $result = '';
        return view('admin/training_evaluation_96_101/list', compact('classArr', 'result'));
    }
    ...
    public function getclass() {
            $sql = "SELECT DISTINCT t53tb.class, t01tb.name
                     ...
                     WHERE t53tb.times<>'' AND SUBSTRING(t53tb.class,1,3) >='096'
    ...
    訓練成效評估表 CSDIR5012
    參考Tables:
    使用範本:L12A.docx , L12B.docx (基本資料列印選項,  L12A 不要包含, L12B 要包含 )
    */
    /**
     * 列印檔案
     *
    ...
    public function export(Request $request)
    {
        ...
        */
        if($ratioInfo!="1")
          $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('../example/'.iconv('UTF-8', 'BIG5', 'L12A').'.docx');
        else
          $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor('../example/'.iconv('UTF-8', 'BIG5', 'L12B').'.docx');
        ...
        */
        if($ratioInfo!="1")
          $outputname="訓練成效評估表(96-101)-不包含基本資料";
        else
          $outputname="訓練成效評估表(96-101)-要包含基本資料";
