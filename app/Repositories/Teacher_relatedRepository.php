...
class Teacher_relatedRepository
{
    /**
     * 取得列表
     *
    ...
    public function getTeacherByWeek($queryData = []){
        // dd($queryData);
        $query = Teacher_by_week::select('teacher_by_week.id', 'teacher_by_week.idno', 'teacher_by_week.confirm', 'teacher_by_week.confirm2', 'teacher_by_week.the_day_before', 'teacher_by_week.the_day_after', 'teacher_by_week.come_by_self', 'teacher_by_week.go_by_self', 'teacher_by_week.drive_by_self', 'teacher_by_week.demand');
    ...
    public function getSpecialNeed($queryData = []){
        ...
        $data = $query->where('class', $queryData['class'])->where('term', $queryData['term'])->get()->toArray();
        // dd($data);
        $specialNeed = array();
        foreach($data as & $row){
            $query = Teacher_by_week::select('idno', 'demand');
            $teacherNeed = $query->where('class_week_id', $row['id'])->get()->toArray();
            foreach($teacherNeed as & $teacherNeed_row){
                $query = M01tb::select('m01tb.cname');
                $m01data = $query->where('idno', $teacherNeed_row['idno'])->get()->toArray();

                if(!empty($m01data)){
                    $teacherNeed_row['name'] = $m01data[0]['cname'];
                }else{
                    $teacherNeed_row['name'] = '無姓名資料';
                }
                $teacherNeed_row['date'] = $row['sdate'].'~'.$row['edate'];

                $specialNeed[] = $teacherNeed_row;
            }
        }
        // dd($specialNeed);
        return $specialNeed;
    ...
    {
        $query = T01tb::select('t01tb.name', 't01tb.yerly');
        $results = $query->where('class', $queryData['class'])->get()->toArray();
        return $results;
    }
